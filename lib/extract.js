...
var Extractor = (function () {
    function Extractor(options) {
        ...
        this.strings = {};
        var keyWords = '(' + this.options.attributes.join('|') + ')';
    ...
    Extractor.prototype.extractJs = function (filename, src, lineNumber) {
        ...
        /* Betterworks specific option */
        if (self.options.paramOrder === undefined) {
            self.options.paramOrder = {
                'text': 0,
                'bindings': 1,
                'plural': 2,
                'context': 3
        ...
        walkJs(syntax, function (node, parentComment) {
            ...
            if (isGettext(node) || isGetString(node) || isGetPlural(node)) {
                str = getJSExpression(node.arguments[self.options.paramOrder.text]);
                if (node.arguments[2]) {
                    plural = getJSExpression(node.arguments[self.options.paramOrder.plural]);
                }
                if (node.arguments[3]) {
                    context = getJSExpression(node.arguments[self.options.paramOrder.context]);
                }
